...
* Relate mass-spec and MIBI measurements by learning a mapping from common
  ...
  - Our predicted spatial expression for a mass spec sample is the spatial
...
ggplot(polys_df %>% filter(SampleID %in% levels(polys_df$SampleID)[1:25])) +
  geom_sf(aes(fill = as.factor(tumorYN))) +
...
We'll finally load the mass spec data to identify shared features. Then, we'll look
at the relationship between those features and the above embeddings, to see if
we could map the mass spec data into the mibitof embedding space.

These are the antigens in both the mass spec and mibitof data.

```{r}
load(file.path(data_dir, "masstagSCE.rda"))
masstag <- data_list("sce") %>%
  subsample_experiments()
masstag_mat <- lapply(masstag, function(x) {
  z <- t(assay(x))
...
sort(colnames(masstag_mat))
rownames(mibi.sce)
common_antigens <- intersect(rownames(masstag$tcell.sce), rownames(mibi.sce))
common_antigens
```

Here are the sample information available in either. It seems that we could use the cell type and the grade of the samples; other than that, the features are not very comparable.
```{r}
names(colData(mibi.sce))
names(colData(masstag$tcell.sce))
```
...
Since I want to predict a two-dimensional response, I'll use a simple MLP with
two output heads.

```{r}
n_ft <- length(common_antigens) + length(levels(col_df$cell_type))
model <- keras_model_sequential() %>%
  layer_dense(units = 16, input_shape = n_ft) %>%
...
model %>%
  fit(as.matrix(predictors), response, epochs=20, batch_size=32)
```
