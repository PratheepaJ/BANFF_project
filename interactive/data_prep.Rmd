...
x$hm_order <- hclust(dist(x))$order
x$cellLabelInImage <- polys$cellLabelInImage

mx <- x %>%
  melt(id.vars = c("cellLabelInImage", "hm_order"), variable.name = "channel")

jsonlite::write_json(mx, file.path("..", "interactive", "map-hm", "assets", "channels.json"))
```
